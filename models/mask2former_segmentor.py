...
class DamageSegmentor(nn.Module):
    ...
    def compute_losses(
        ...
        # Read class weights from config if provided; otherwise use a binary default.
        class_weights_cfg = self.loss_config.get("class_weights")
        if class_weights_cfg is not None:
            class_weights = torch.tensor(
                class_weights_cfg, device=logits.device, dtype=logits.dtype
            )
        elif logits.shape[1] == 2:
            class_weights = torch.tensor([1.0, 4.0], device=logits.device, dtype=logits.dtype)
        else:
            class_weights = None
        ...
        if use_gradient or loss_name in {"grad", "gradient", "gradient_boundary"}:
            grad = gradient_boundary_loss(
                logits, targets, foreground_class_index=grad_class_index
            )
        else:
            grad = logits.new_zeros(())
