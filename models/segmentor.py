...
from models.backbone.mask2former import Mask2FormerBackbone
...
class DamageSegmentor(nn.Module):
    def __init__(
        self,
        num_classes: int,
        pretrained_model_name: str = "facebook/mask2former-swin-small-ade-semantic",
        loss_config: Dict | None = None,
        # Compatibility args (ignored): older training/eval code passes these.
        in_channels: int = 3,
        ...
        # Mask2Former semantic segmentation already returns dense per-class logits.
        self.backbone = Mask2FormerBackbone(
    ...
    def forward(self, images: torch.Tensor) -> Dict[str, torch.Tensor]:
        """
        Args:
            images: normalized float tensor [B, 3, H, W]
        Returns:
            logits in shape [B, num_classes, H, W]
        """
    ...
    def compute_losses(
        ...
        # Segmentation-only losses (CE / Dice / Focal / Gradient).
        ...
        grad = gradient_boundary_loss(logits, targets, foreground_class_index=grad_class_index)

        # Unsupported losses: return zeros so training/eval code can keep a stable schema.
        loss_contrastive = logits.new_zeros(())
        loss_cls = logits.new_zeros(())
        loss_cls_contrastive = logits.new_zeros(())
        ...
        # Optionally add gradient boundary loss as an auxiliary term.
        ...
        return {
            ...
            "loss_cls": loss_cls,
            "loss_cls_contrastive": loss_cls_contrastive,
        }
