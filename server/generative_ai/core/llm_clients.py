...
class QwenVLLLMClient(BaseLLMClient):
    ...
    def _build_prompt_text(self, prompt_vars: dict) -> str:
        ...
        safe_vars = {k: str(prompt_vars.get(k, "[PLACEHOLDER]")) for k in expected_keys}
        return CLAIM_DRAFT_SYSTEM_PROMPT.format(**safe_vars)
    ...
    def generate_raw(self, prompt: str, images=None, max_tokens=1000) -> str:
        """
        Generate text from a plain prompt string + optional images.
        Used for chained single-section calls from app.py.
        """
        import torch
        from qwen_vl_utils import process_vision_info
